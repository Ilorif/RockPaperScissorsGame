 ...
 *    Exercise: 2
 *      Author: Ilor Ifrach & Yogev Neumann
	...
	for (i=1;i<=M_COLS;++i) { // M_COLS is x
		for (j=1;j<=N_ROWS;++j) { // N_ROWS is y
			...
			if ((player_1!=EMPTY)&&(player_2!=EMPTY)) { // We have a fight
				...
				std::unique_ptr<FightInfoRPS> unique_fight = std::make_unique<FightInfoRPS>(PointRPS(i,j), hideJokerTypeRep(player_1), hideJokerTypeRep(player_2), loop_fight_winner); // hideJokerTypeRep() Return R, P, S, B or F
...
std::unique_ptr<FightInfoRPS> GameManager::fightMove(char piece_from, char piece_to, const Point& from, const Point& to) {
	...
	std::unique_ptr<FightInfoRPS> unique_fight = std::make_unique<FightInfoRPS>(to,hideJokerTypeRep(piece_from),hideJokerTypeRep(piece_to),winner_player); // hideJokerTypeRep() Return R, P, S, B or F
	return unique_fight;
}
		...
		for (i=1;i<=M_COLS;++i) { // M_COLS is x
			for (j=1;j<=N_ROWS;++j) { // N_ROWS is y
				if (board_player1[i-1][j-1]!=EMPTY) { // 1<=i<=M 1<=j<=N
					std::unique_ptr<PiecePositionRPS> unique_piece = std::make_unique<PiecePositionRPS>(board_player1[i-1][j-1],PointRPS(i, j),1);
					board.addPiecePosition(unique_piece,1);
				} else if (board_player2[i-1][j-1]!=EMPTY) { // 1<=i<=M 1<=j<=N
					std::unique_ptr<PiecePositionRPS> unique_piece = std::make_unique<PiecePositionRPS>(board_player2[i-1][j-1],PointRPS(i, j),2);
...
int GameManager::jokerCommand(JokerChange& joker_change) {
	...
	char old_piece = board.getPiece(PointRPS(x,y)); // GameManagerBoard.getPiece() Return R, P, S, B, Joker type, F or EMPTY
	if ((x<1)||(x>M_COLS)) {
...
int GameManager::positioning(PiecePosition& piece_position, int player) {
	...
	char is_joker = piece_position.getJokerRep(); // ONLY for Joker: R, P, S or B -- non-Joker return #
	if (validation(piece_position,player)==0) { // validation() Return 0 if valid and 1 otherwise
		if ((is_joker==ROCK)||(is_joker==PAPER)||(is_joker==SCISSORS)||(is_joker==BOMB)) { // Piece is joker // getJokerRep() return ONLY for Joker: R, P, S or B -- non-Joker may return #
...
void GameManager::printBoard(std::ofstream& output_file) {
	...
	for (j=1;j<=N_ROWS;++j) { // N_ROWS is y
		for (i=1;i<=M_COLS;++i) { // M_COLS is x
			...
			piece_at = board.getPiece(loop_point); // GameManagerBoard.getPiece() Return R, P, S, B, Joker type, F or EMPTY
...
int GameManager::validateMove(const Point& from, const Point& to) {
	...
	char from_piece = board.getPiece(from); // GameManagerBoard.getPiece() Return R, P, S, B, Joker type, F or EMPTY
...
int GameManager::validation(PiecePosition& piece_position, int player) {
	...
	if ((is_joker==ROCK)||(is_joker==PAPER)||(is_joker==SCISSORS)||(is_joker==BOMB)) { // Piece is joker // getJokerRep() return ONLY for Joker: R, P, S or B -- non-Joker may return #
		if ((is_joker!=ROCK)&&(is_joker!=PAPER)&&(is_joker!=SCISSORS)&&(is_joker!=BOMB)) {
			std::cout << ERROR_INVALID_JOKER << is_joker << "'" << std::endl; // "ERROR: Invalid joker type '?'"
			return 1;
		}
...
int GameManager::play(std::unique_ptr<PlayerAlgorithm> players[2], int &return_value) {
	int res;
	...
	// Positioning
	players[0].get()->getInitialPositions(1, init_player1);
	players[1].get()->getInitialPositions(2, init_player2);
	init_fights = initialize(res,init_player1,init_player2); // initialize() Set ret to 0 if success and 1 otherwise, Return FightInfo vector of all the positioning fights
	if (res==1) { // Error message already been set by initialize()
		...
		//createOutputFile(); // Comment because of http://moodle.tau.ac.il/mod/forum/discuss.php?d=70378#p103244
		return 1;
	}
	BoardRPS init_board = *(getBoard().get()); // getBoard() Create Board object based on the current game board
	players[0].get()->notifyOnInitialBoard(init_board,init_fights);
	players[1].get()->notifyOnInitialBoard(init_board,init_fights);
	// Moves
	while ((checkGameOver()==1)&&(no_fight_counter<NO_FIGHTS)) { // checkGameOver() Return 1 if game is still on and 0 otherwise
		no_fight_counter++;
		turn_move = players[getCurrentPlayer()-1].get()->getMove();
		if (turn_move!=nullptr) {
			turn_fight = move(*(turn_move.get()),res); // move() Return FightInfo if there was a fight and nullptr otherwise, Set res to 0 if success and 1 otherwise
		}
		if (last_player_EOF==1) { // Last player does not have any more moves
			if (turn_move==nullptr) { // Tie, Both players does not have any more moves
				setGameOver(0,REASON_TIE_NO_MORE_MOVES); // "A tie - both Moves input files done without a winner"
			} else { // Current player win, Last player does not have any more moves
				std::cout << ERROR_PLAYER_NUM << this->getCurrentPlayer() << ERROR_NO_MORE_MOVES_B << std::endl; // "ERROR: Player ? don't have any more moves"
				setGameOver(getCurrentPlayer(),REASON_BAD_MOVES); // "Bad Moves input file for player ?"
			}
			...
			std::cout << ERROR_PLAYER_NUM << getCurrentPlayer() << ERROR_BAD_MOVE_B << std::endl; // "ERROR: Player ? did invalid move"
			setGameOver(getOtherPlayer(),REASON_BAD_MOVES); // "Bad Moves input file for player ?"
		} else { // Player 1 move was fine
			last_player_EOF = 0;
			if (turn_fight!=nullptr) {
				players[getCurrentPlayer()-1].get()->notifyFightResult(*(turn_fight.get())); // notifyFightResult() Called only if there was a fight
				no_fight_counter = 0;
			}
			turn_jokerchange = players[getCurrentPlayer()-1].get()->getJokerChange(); // getJokerChange() Return nullptr if no change is requested
			if (turn_jokerchange!=nullptr) {
				if (jokerCommand(*(turn_jokerchange.get()))==1) { // jokerCommand() Return 0 on success and 1 otherwise
			...
			players[1-(getCurrentPlayer()-1)].get()->notifyOnOpponentMove(*(turn_move.get()));
			if (turn_fight!=nullptr) {
				players[1-(getCurrentPlayer()-1)].get()->notifyFightResult(*(turn_fight.get())); // notifyFightResult() Called only if there was a fight
			}
		}
		changePlayer();
	}
