...
////	RSPPlayer_123456789 					////
////////////////////////////////////////////////////////////////////
RSPPlayer_123456789::RSPPlayer_123456789() {
	int i,j; // 1<=i<=M 1<=j<=N
...
////////////////////////////////////////////////////////////////////
void RSPPlayer_123456789::getInitialPositions(int player, std::vector<std::unique_ptr<PiecePosition>>& vectorToFill) {
	...
	// Defenders
	vectorToFill.push_back(std::make_unique<PiecePositionRPS>(FLAG,PointRPS(flag_x+mapping[side][0][0],flag_y+mapping[side][0][1]),whoami)); // Flag
	board_my[flag_x+mapping[side][0][0]-1][flag_y+mapping[side][0][1]-1][0] = FLAG;
	vectorToFill.push_back(std::make_unique<PiecePositionRPS>(BOMB,PointRPS(flag_x+mapping[side][1][0],flag_y+mapping[side][1][1]),whoami)); // Bomb 1/2
	board_my[flag_x+mapping[side][1][0]-1][flag_y+mapping[side][1][1]-1][0] = BOMB;
	vectorToFill.push_back(std::make_unique<PiecePositionRPS>(BOMB,PointRPS(flag_x+mapping[side][2][0],flag_y+mapping[side][2][1]),whoami)); // Bomb 2/2
	board_my[flag_x+mapping[side][2][0]-1][flag_y+mapping[side][2][1]-1][0] = BOMB;
	vectorToFill.push_back(std::make_unique<PiecePositionRPS>(JOKER_SCISSORS,PointRPS(flag_x+mapping[side][3][0],flag_y+mapping[side][3][1]),whoami)); // Joker 1/2 as Scissors
	board_my[flag_x+mapping[side][3][0]-1][flag_y+mapping[side][3][1]-1][0] = JOKER_SCISSORS;
	vectorToFill.push_back(std::make_unique<PiecePositionRPS>(JOKER_SCISSORS,PointRPS(flag_x+mapping[side][4][0],flag_y+mapping[side][4][1]),whoami)); // Joker 2/2 as Scissors
	board_my[flag_x+mapping[side][4][0]-1][flag_y+mapping[side][4][1]-1][0] = JOKER_SCISSORS;
	vectorToFill.push_back(std::make_unique<PiecePositionRPS>(PAPER,PointRPS(flag_x+mapping[side][5][0],flag_y+mapping[side][5][1]),whoami)); // paper 1/5
	board_my[flag_x+mapping[side][5][0]-1][flag_y+mapping[side][5][1]-1][0] = PAPER;
	vectorToFill.push_back(std::make_unique<PiecePositionRPS>(PAPER,PointRPS(flag_x+mapping[side][6][0],flag_y+mapping[side][6][1]),whoami)); // paper 2/5
	board_my[flag_x+mapping[side][6][0]-1][flag_y+mapping[side][6][1]-1][0] = PAPER;
	vectorToFill.push_back(std::make_unique<PiecePositionRPS>(ROCK,PointRPS(flag_x+mapping[side][7][0],flag_y+mapping[side][7][1]),whoami)); // Rock 1/2
	board_my[flag_x+mapping[side][7][0]-1][flag_y+mapping[side][7][1]-1][0] = ROCK;
	// Fighters
	vectorToFill.push_back(std::make_unique<PiecePositionRPS>(PAPER,getAvailablePoint(PAPER),whoami)); // paper 3/5 // getAvailablePoint() Return available point at the board
	vectorToFill.push_back(std::make_unique<PiecePositionRPS>(PAPER,getAvailablePoint(PAPER),whoami)); // paper 4/5 // getAvailablePoint() Return available point at the board
	vectorToFill.push_back(std::make_unique<PiecePositionRPS>(PAPER,getAvailablePoint(PAPER),whoami)); // paper 5/5 // getAvailablePoint() Return available point at the board
	vectorToFill.push_back(std::make_unique<PiecePositionRPS>(ROCK,getAvailablePoint(ROCK),whoami)); // Rock 2/2 // getAvailablePoint() Return available point at the board
	vectorToFill.push_back(std::make_unique<PiecePositionRPS>(SCISSORS,getAvailablePoint(SCISSORS),whoami)); // Scissors 1/1 // getAvailablePoint() Return available point at the board
}

void RSPPlayer_123456789::notifyOnInitialBoard(const Board& b, const std::vector<std::unique_ptr<FightInfo>>& fights) {
	int i,j; // 1<=i<=M 1<=j<=N
	int player_at;
...
void RSPPlayer_123456789::notifyOnOpponentMove(const Move& move) { // Called only on opponent's move
...
void RSPPlayer_123456789::notifyFightResult(const FightInfo& fightInfo) { // Called only if there was a fight
	int x,y,winner;
...
unique_ptr<Move> RSPPlayer_123456789::getMove() {
...
////////////////////////////////////////////////////////////////////
////	RSPPlayer_123456789 functions 			////
////////////////////////////////////////////////////////////////////
// Return 0 if game is over for us and 1 otherwise
int RSPPlayer_123456789::checkGameOver() {
	int i,j; // 1<=i<=M 1<=j<=N
	for (i=1;i<=M_COLS;++i) { // M_COLS is x
		for (j=1;j<=N_ROWS;++j) { // N_ROWS is y
...
int RSPPlayer_123456789::fightSimulate(char piece_1, char piece_2) {
	int player_1_lose = 1;
	int player_2_lose = 1;
	switch (piece_1) {
...
// Guess what is the opponent's flag
void RSPPlayer_123456789::findTarget() {
	std::random_device rd;
	std::mt19937 mt(rd());
...
PointRPS RSPPlayer_123456789::findVictim(PointRPS my_piece) {
	int i,j; // 1<=i<=M 1<=j<=N
	for (i=1;i<=M_COLS;++i) { // M_COLS is x
		for (j=1;j<=N_ROWS;++j) { // N_ROWS is y
...
PointRPS RSPPlayer_123456789::getAvailablePoint(char piece) {
...
void RSPPlayer_123456789::minMax() {
	int i,j; // 1<=i<=M 1<=j<=N
	int left,right,down,up;
	int max_score = 0;
	PointRPS pos_source = PointRPS(-3,-3);
	PointRPS pos_direction = PointRPS(-4,-4);
	for (i=1;i<=M_COLS;++i) { // M_COLS is x
		for (j=1;j<=N_ROWS;++j) { // N_ROWS is y
...
double RSPPlayer_123456789::scoring(PointRPS source_piece, PointRPS next_step) {
